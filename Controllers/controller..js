...
var connectionString = "postgres://postgres:Greg366.@localhost:5432/teamwork";
    ...
    client.query(`SELECT * FROM account WHERE user_id = ${decoded.userId}`, (err, result) => {
        ...
        .then(encryptedPassword => {
            client.query(`INSERT INTO account(first_name,last_name,email,password,job_role,department,address) VALUES('${req.query.fname}','${req.query.lname}','${req.query.email}','${encryptedPassword}','${req.query.jobrole}','${req.query.department}','${req.query.address}')`, (err, result) => {
                res.status(200,'successful').send(`${result} or err is ${err} and pass is ${encryptedPassword}`)
...
exports.createNewGif = (req , res) => {
    ...
    try{
        const decoded = jwt.verify(token, 'secret')
        client.query(`INSERT INTO gifs(author_id, gif_title, image_url, created_on, category) VALUES('${decoded.userId}','${req.query.title}','${req.query.image_url}','${dateCreated}','${req.query.category}') `, (err, result) => {
